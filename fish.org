	...
	with each element matching a rule or goal of my prompt.
	 ...
	 #+BEGIN_SRC fish :tangle yes :tangle config.fish
		 function fish_prompt
				 # Store the exit code of the last ran command. If I don't do this here, then
				 # getting the exit code later can result in getting the exit code from
				 # one of the __prompt functions.
				 # The exit code is used to determin the colour of the status indicator.
				 set -l last_exit_code $status

				 echo # Just gives more breathing room.

				 __prompt_username_host_name
				 __prompt_directory_path
				 __prompt_git_branch
				 __prompt_git_status_state
				 __prompt_status_indicator "$last_exit_code"

				 set_color normal # Ensure the colour is reset
		 end
	 #+END_SRC
...
** The Git Branch 
	 ...
	 1. Given the current directory is part if a Git repo, when I am inside a
			...
			and display the branch name in magenta.

	 #+BEGIN_SRC fish :tangle yes :tangle config.fish
		 function __prompt_git_branch
				 # Show the Git branch if the current directory is part of a Git repo.
				 # The native Git prompt functionality of Fish does display the repo,
				 # but not quite in the way I want, so I handle this myself.
				 set -l is_git_repo (git rev-parse --is-inside-working-tree \
				 >/dev/null 2>&1; echo $status)

				 if test $is_git_repo -eq 0
						 set_color normal; echo -n "on "
						 set_color magenta; echo -n " "(git symbolic-ref --short HEAD)
				 end
		 end
	 #+END_SRC
...
** The Git Status & State
	 ...
	 #+BEGIN_SRC fish :tangle yes :tangle config.fish
		 # Set the symbols and information shown in the prompt. I enable the built-in
		 # informative state, which sets some nicer symbols as well as a total count
		 # for each file in the given status. I also replace some symbols to match
		 # what is used by the Starship prompt.
		 set __fish_git_prompt_show_informative_status 'yes'
		 set __fish_git_prompt_char_upstream_diverged '⇕'
		 set __fish_git_prompt_char_untrackedfiles '?'
		 set __fish_git_prompt_char_dirtystate '!'
		 set __fish_git_prompt_char_statgedstate '+'

		 function __prompt_git_status_state
				 set -l is_git_repo (git rev-parse --is-inside-working-tree \
				 >/dev/null 2>&1; echo $status)

				 if test $is_git_repo -eq 0
						 # This is a hack and I hate it. I want to leverage the native Git prompt
						 # function from Fish, but there is not way to exclude the current branch
						 # from being displayed.
						 # So what I do is grab the current branches name and strip it from the
						 # results of the `__fish_git_prompt` function. It works, but it feels
						 # so hamfisted.
						 set current_branch (git symbolic-ref --short HEAD)
						 set -l prompt_result (__fish_git_prompt)
						 set -l prompt_result \
						 (string replace "$current_branch""|" "" "$prompt_result")

						 # This is equally terrible.
						 set -l prompt_result (string replace "(" "[" "$prompt_result")
						 set -l prompt_result (string replace ")" "]" "$prompt_result")

						 set_color --dim --italics white; echo "$prompt_result"
				 end
		 end
	 #+END_SRC
...
** The Status Indicator
	 ...
	 #+BEGIN_SRC fish :tangle yes :tangle config.fish
		 function __prompt_status_indicator
				 set -l prompt_symbol '❯ '

				 if test "$argv[1]" -eq 0
						 set_color brgreen; echo -n $prompt_symbol
				 else
						 set_color brred; echo -n $prompt_symbol
				 end
		 end
	 #+END_SRC
